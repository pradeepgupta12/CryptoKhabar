...
import { cryptoCurrencyNews } from '../data/cryptoCurrencyNews'; // Adjust path to your cryptoCurrencyNews.js file
import { useEffect } from 'react';

function LatestCryptoCurrencyNewsDetailsPage() {
  ...
  // Use the article from state, or fallback to the first crypto currency news item
  const article = state?.article || cryptoCurrencyNews[0] || {};

  // Structured data for the article
  const articleSchema = {
    "@context": "https://schema.org",
    "@type": "NewsArticle",
    "headline": article.title || 'Untitled Crypto News',
    ...
    "description": article.description || 'Latest cryptocurrency news and updates.',
    "author": {
      "@type": "Person",
  ...
  const shareArticle = (e) => {
    ...
    if (navigator.share) {
      navigator.share({
        title: article?.title || 'CryptoNews Article',
  ...
  const handleArticleClick = (relatedArticle) => {
    ...
    navigate('/latest-crypto-currency-news', { 
  ...
  return (
    <div className="w-full min-h-screen bg-gray-100 pt-12 p-4 mt-8">
      ...
      <Helmet>
        ...
        <title>{`${article.title || 'Crypto News'} - Crypto Khabar`}</title>
        <meta
          name="description"
          content={article.description || `Read the latest cryptocurrency news: ${article.title || 'Crypto News'} on Crypto Khabar.`}
        />
        <meta name="keywords" content={`cryptocurrency news, crypto, ${article.title ? article.title.toLowerCase() : ''}, Crypto Khabar`} />
        <meta name="author" content={article.author || "Crypto Khabar Team"} />
        <link rel="icon" href="/favicon.ico" />
        <link rel="canonical" href={`https://cryptokhabar.net/latest-crypto-currency-news/${article.id}`} />
        <meta name="robots" content="index, follow" />
        <meta property="og:title" content={`${article.title || 'Crypto News'} - Crypto Khabar`} />
        <meta property="og:description" content={article.description || `Read the latest cryptocurrency news: ${article.title || 'Crypto News'} on Crypto Khabar.`} />
        <meta property="og:type" content="article" />
        <meta property="og:url" content={`https://cryptokhabar.net/latest-crypto-currency-news/${article.id}`} />
            ...
            <article className="bg-white rounded-lg shadow-lg overflow-hidden">
              <div className="relative">
                <img
                  ...
                  alt={`Featured image for ${article.title || 'Crypto News'} in cryptocurrency news`}
              ...
              <div className="p-6 sm:p-8">
                <h1 className="text-2xl sm:text-2xl font-bold text-gray-900 mb-4">
                  {article.title || 'Untitled Crypto News'}
                </h1>
              ...
              .map((story) => (
                ...
                >
                  <img
                    ...
                    alt={`Image for ${story.title || 'Related Crypto News'} in cryptocurrency news`}
...
export default LatestCryptoCurrencyNewsDetailsPage;