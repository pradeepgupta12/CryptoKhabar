...
import nftData from '../data/nftData'; // Adjust path to your nftData.js file

function NFTsDetailsPage() {
  ...
  const item = state?.item || nftData.latest_nft_news.articles[0] || {};
  const type = state?.type || 'article'; // 'story' or 'article'

  // Structured data for the NFT news item
  const articleSchema = {
    "@context": "https://schema.org",
    "@type": "NewsArticle",
    "headline": item.headline || 'Untitled NFT News',
    "image": item.image || 'https://via.placeholder.com/500x500',
    "datePublished": item.time || '2025-09-11',
  ...
  const shareArticle = (e) => {
    ...
    if (navigator.share) {
      navigator.share({
        title: item.headline || 'NFT News',
        text: 'Check out this NFT news article',
  ...
  const handleRelatedItemClick = (relatedItem, relatedType) => {
    ...
    navigate(`/nfts/${relatedItem.id}`, { state: { item: relatedItem, type: relatedType } });
    window.scrollTo(0, 0); // Scroll to top on navigation
  };

  // Combine stories and articles for related items, excluding the current item
  const relatedItems = [
    ...nftData.top_nft_stories.stories.map((story) => ({ ...story, type: 'story' })),
    ...nftData.latest_nft_news.articles.map((article) => ({ ...article, type: 'article' })),
  ...
  return (
    <div className="w-full min-h-screen bg-gray-100 pt-12 p-4 mt-8">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <article className="bg-white rounded-lg shadow-lg overflow-hidden">
              <div className="relative">
                <img
                  ...
                  alt={`Featured image for ${item.headline || 'NFT News'} in NFT news`}
              ...
              <div className="p-6 sm:p-8">
                <h1 className="text-2xl sm:text-2xl font-bold text-gray-900 mb-4">
                  {item.headline || 'Untitled NFT News'}
                </h1>
                ...
                <div className="mb-6">
                  ...
                  <p className="text-gray-600 text-base sm:text-lg leading-relaxed">
                    {item.summary
                      ...
                      : 'Detailed information about this NFT news is currently unavailable. Stay tuned for updates.'}
                  </p>
                </div>
              </div>
            </article>
        ...
        <div className="mt-8">
          <h2 className="text-2xl font-bold text-gray-900 mb-4">Related NFT News</h2>
            ...
            {relatedItems.map((related) => (
              ...
              >
                <img
                  ...
                  alt={`Image for ${related.headline || 'Related NFT News'} in NFT news`}
