...
function EthereumDetailsPage() {
  ...
  const shareArticle = (e) => {
    ...
    if (navigator.share) {
      navigator.share({
        title: item.headline || 'Ethereum News',
        text: 'Check out this Ethereum news article',
  ...
  const handleRelatedItemClick = (relatedItem, relatedType) => {
    ...
    navigate(`/ethereum/${relatedItem.id}`, { state: { item: relatedItem, type: relatedType } });
    window.scrollTo(0, 0); // Scroll to top on navigation
  };

  // Combine stories and articles for related items, excluding the current item
  const relatedItems = [
    ...ethereumData.top_ethereum_stories.stories.map((story) => ({ ...story, type: 'story' })),
    ...ethereumData.latest_ethereum_news.articles.map((article) => ({ ...article, type: 'article' })),
  ...
  return (
    <div className="w-full min-h-screen bg-gray-100 pt-12 p-4 mt-8">
      ...
      <Helmet>
        ...
        />
        <meta name="keywords" content={`ethereum news, cryptocurrency, blockchain, ${item.headline ? item.headline.toLowerCase() : ''}, Crypto Khabar`} />
        <meta name="author" content="Crypto Khabar Team" />
        <link rel="icon" href="/favicon.ico" />
        <link rel="canonical" href={`https://cryptokhabar.net/ethereum/${item.id}`} />
        <meta name="robots" content="index, follow" />
        <meta property="og:title" content={`${item.headline || 'Ethereum News'} - Crypto Khabar`} />
        <meta property="og:description" content={item.summary || `Read the latest Ethereum news: ${item.headline || 'Ethereum News'} on Crypto Khabar.`} />
        <meta property="og:type" content="article" />
        <meta property="og:url" content={`https://cryptokhabar.net/ethereum/${item.id}`} />
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <article className="bg-white rounded-lg shadow-lg overflow-hidden">
              ...
              <div className="p-6 sm:p-8">
                <h1 className="text-2xl sm:text-2xl font-bold text-gray-900 mb-4">
                  {item.headline || 'Untitled Ethereum News'}
                </h1>
                ...
                <div className="mb-6">
                  ...
                  <p className="text-gray-600 text-base sm:text-lg leading-relaxed">
                    {item.summary
                      ...
                      : 'Detailed information about this Ethereum news is currently unavailable. Stay tuned for updates.'}
                  </p>
                </div>
              </div>
            </article>
        ...
        <div className="mt-8">
          <h2 className="text-2xl font-bold text-gray-900 mb-4">Related Ethereum News</h2>
...
export default EthereumDetailsPage;