...
import blockchainData from '../data/blockchainData'; // Adjust path to your blockchainData.js file

function BlockChainDetailsPage() {
  ...
  const item = state?.item || blockchainData.latest_blockchain_news.articles[0] || {};
  ...
  const articleSchema = {
    ...
    "@type": "NewsArticle",
    "headline": item.headline || 'Untitled Blockchain News',
  ...
  const shareArticle = (e) => {
    ...
    if (navigator.share) {
      navigator.share({
        title: item.headline || 'Blockchain News',
        text: 'Check out this blockchain news article',
  ...
  const handleRelatedItemClick = (relatedItem, relatedType) => {
    ...
    navigate(`/blockchain/${relatedItem.id}`, { state: { item: relatedItem, type: relatedType } });
    window.scrollTo(0, 0); // Scroll to top on navigation
  };

  // Combine stories and articles for related items, excluding the current item
  const relatedItems = [
    ...blockchainData.top_blockchain_stories.stories.map((story) => ({ ...story, type: 'story' })),
    ...blockchainData.latest_blockchain_news.articles.map((article) => ({ ...article, type: 'article' })),
  ...
  return (
    <div className="w-full min-h-screen bg-gray-100 pt-12 p-4 mt-8">
      ...
      <Helmet>
        ...
        <title>{`${item.headline || 'Blockchain News'} - Crypto Khabar`}</title>
        <meta
          name="description"
          content={item.summary || `Read the latest Blockchain news: ${item.headline || 'Blockchain News'} on Crypto Khabar.`}
        />
        <meta name="keywords" content={`blockchain news, cryptocurrency, blockchain, ${item.headline ? item.headline.toLowerCase() : ''}, Crypto Khabar`} />
        <meta name="author" content="Crypto Khabar Team" />
        <link rel="icon" href="/favicon.ico" />
        <link rel="canonical" href={`https://cryptokhabar.net/blockchain/${item.id}`} />
        <meta name="robots" content="index, follow" />
        <meta property="og:title" content={`${item.headline || 'Blockchain News'} - Crypto Khabar`} />
        <meta property="og:description" content={item.summary || `Read the latest Blockchain news: ${item.headline || 'Blockchain News'} on Crypto Khabar.`} />
        <meta property="og:type" content="article" />
        <meta property="og:url" content={`https://cryptokhabar.net/blockchain/${item.id}`} />
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <article className="bg-white rounded-lg shadow-lg overflow-hidden">
              <div className="relative">
                <img
                  ...
                  alt={`Featured image for ${item.headline || 'Blockchain News'} in Blockchain news`}
              ...
              <div className="p-6 sm:p-8">
                <h1 className="text-2xl sm:text-2xl font-bold text-gray-900 mb-4">
                  {item.headline || 'Untitled Blockchain News'}
                </h1>
                ...
                <div className="mb-6">
                  ...
                  <p className="text-gray-600 text-base sm:text-lg leading-relaxed">
                    {item.summary
                      ...
                      : 'Detailed information about this blockchain news is currently unavailable. Stay tuned for updates.'}
                  </p>
                </div>
              </div>
            </article>
        ...
        <div className="mt-8">
          <h2 className="text-2xl font-bold text-gray-900 mb-4">Related Blockchain News</h2>
            ...
            {relatedItems.map((related) => (
              ...
              >
                <img
                  ...
                  alt={`Image for ${related.headline || 'Related Blockchain News'} in Blockchain news`}
